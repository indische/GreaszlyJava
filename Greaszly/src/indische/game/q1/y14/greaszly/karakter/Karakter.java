...
public class Karakter {
	...
	private float animationVar = 0.05f;
	...
	public void create() {
		...
		face_e_rec = new Rectangle(karakter_rec.x + (karakter_rec.width / 2)
				- 20, karakter_rec.y + (karakter_rec.height / 3) - 6,
	...
	public void update() {
		...
		boingBoingVar = stateTime / Karakter.stamina_BoingVar / 2;
		...
		if (karakter_rec.x <= 70) {
			...
			face_e_rec.x = karakter_rec.x + (karakter_rec.width / 2) - 20;
		}
	...
	public void draw(SpriteBatch batch) {
		...
		if (boing < 0)
			boing *= -1;
		float boingLevel = (float) (boing * 8);

		// masked limbs
		tangKiri_frac = tangKiri_ani.getKeyFrame(organVar, true);
		batch.draw(tangKiri_frac, tangKiri_rec.x, tangKiri_rec.y - boingLevel,
				tangKiri_rec.width, tangKiri_rec.height);

		kakiKiri_frac = kakiKiri_ani.getKeyFrame(organVar, true);
		batch.draw(kakiKiri_frac, kakiKiri_rec.x, kakiKiri_rec.y - boingLevel,
				kakiKiri_rec.width, kakiKiri_rec.height);

		// <body>
		batch.draw(karakter_tex, karakter_rec.x, karakter_rec.y - boingLevel,
				karakter_rec.width, karakter_rec.height - boingLevel);

		// <face>
		batch.draw(face_e_neut_tex, face_e_rec.x, face_e_rec.y - boingLevel,
				face_e_rec.width, face_e_rec.height);

		// appeared limbs
		tangKanan_frac = tangKanan_ani.getKeyFrame(organVar, true);
		batch.draw(tangKanan_frac, tangKanan_rec.x, tangKanan_rec.y
				- boingLevel, tangKanan_rec.width, tangKanan_rec.height);

		kakiKanan_frac = kakiKanan_ani.getKeyFrame(organVar, true);
		batch.draw(kakiKanan_frac, kakiKanan_rec.x, kakiKanan_rec.y
				- boingLevel, kakiKanan_rec.width, kakiKanan_rec.height);
